...
//        $this->middleware('auth');
     ...
     */
    public function bienBan(Request $request)
    {
        //
        $this->v['_title'] = 'Biên Bản';
        $this->v['routeIndexText'] = 'Biên Bản';
        $objBienBan = new BienBan();
        $this->v['extParams'] = $request->all();
        $this->v['list'] = $objBienBan->loadListWithPager($this->v['extParams']);
        ...
        return view('bienban.bien-ban', $this->v);
    }
    /**
     ...
     * @return \Illuminate\Http\Response
     */
    public function themBienBan(BienBanRequest $request)
    {
        //
        //
        $this->v['routeIndexText'] = 'Thêm biên bản';
        $method_route = 'route_BackEnd_BienBan_Add';
//        $this->v['request'] = Session::pull('post_form_data')[0];
//        SpxGetSsMessage($this->v);
        $this->v['_action'] = 'Add';
        $this->v['_title'] = 'Thêm biên bản';
...
//            }
            $objBienBan = new  BienBan();
            $res = $objBienBan->saveNew($params); // hàm trả về ID mới nếu insert thành công
            if ($res == null) // chuyển trang vì trong session đã có sẵn câu thông báo lỗi rồi
            {
                ...
                Session::flash('success', 'Thêm mới thành công biên bản !');
...
//        $this->v['request'] = Session::pull('post_form_data')[0];
     ...
     * @return \Illuminate\Http\Response
     */
    public function updateChiTietBienBan($id, BienBanRequest $request)
    {
        //
        ...
        $params['cols']['id'] = $id;
        $res = $objBienBan->saveUpdate($params);
...
//        $params = [
//            'user_edit' => Auth::user()->id];
//        $res = $objBienBan->saveDelete($params);
//
//        if ($res == null) // chuyển trang vì trong session đã có sẵn câu thông báo lỗi rồi
//        {
//            Session::flash('success', 'Cập nhật bản ghi: ' . $objItem->id . ' thành công!');
