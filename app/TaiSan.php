...
class TaiSan extends Model
{
    ...
    protected $fillable = ['tb1.id','tb1.ma_tai_san','tb1.ten_tai_san','tb1.danh_muc_tai_san_id','tb1.trang_thai','tb1.created_at','tb1.updated_at'];
...
//            ->select( array_merge($this->fillable,['tb2.username as uName', 'tb2.fullname as hoten_tuvan']))
    ...
    public function saveNew($params)
    {
        ...
        $latestTaiSan = DB::table('tai_san')->orderBy('id','DESC')->first();
        $latestID = $latestTaiSan ? $latestTaiSan->id : 0;
        $maTaiSan = 'TS'.str_pad($latestID + 1, 5, "0", STR_PAD_LEFT);
        $data =  array_merge($params['cols'],[
            'ma_tai_san' => $maTaiSan,
            'ten_tai_san' => $params['cols']['ten_tai_san'],
            'danh_muc_tai_san_id' => $params['cols']['danh_muc_tai_san_id'],
...
//        $params = ['cols'=>['col'=>'new_value'],'user_edit'=>'id'];
    ...
    {
        $list = DB::table($this->table)->select('id', 'ten_tai_san','trang_thai');
        if($where != null)
            $list->where([$where]);
        return $list->get();
    }
