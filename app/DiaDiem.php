...
class DiaDiem extends Model
{
    ...
    protected $fillable = ['tb1.id','tb1.ten_dia_diem','tb1.trang_thai','tb1.created_at','tb1.updated_at'];
    ...
    public function loadListIdAndName($where = null){
        $list = DB::table($this->table)->select('id', 'ten_dia_diem','trang_thai');
        if($where != null)
            $list->where([$where]);
        return $list->get();
    }
    public function saveNew($params){
        ...
        $data = array_merge($params['cols'],['ten_dia_diem' => $params['cols']['ten_dia_diem'],
            'trang_thai' => 1,
