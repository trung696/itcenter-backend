...
class DonVi extends Model
{
    ...
    protected $fillable = ['tb1.id','tb1.ma_don_vi','tb1.ten_don_vi','tb1.trang_thai','tb1.created_at','tb1.updated_at'];
    ...
    public function loadListWithPager($params = array())
    {
        ...
        if (isset($params['search_ten_don_vi']) && strlen($params['search_ten_don_vi']) > 0) {
            $query
                ->where('tb1.ten_don_vi', 'like', '%' . $params['search_ten_don_vi'] . '%');
        }
...
//            ->select( array_merge($this->fillable,['tb2.username as uName', 'tb2.fullname as hoten_tuvan']))
    ...
    public function saveNew($params)
    {
        ...
        $latestDonVi = DB::table('don_vi')->orderBy('created_at','DESC')->first();
        $latestID = $latestDonVi ? $latestDonVi->id : 0;
        $maDonVi = 'DV'.str_pad($latestID + 1, 5, "0", STR_PAD_LEFT);
        $data =  array_merge($params['cols'],[
            'ma_don_vi' => $maDonVi,
            'ten_don_vi' => $params['cols']['ten_don_vi'],
