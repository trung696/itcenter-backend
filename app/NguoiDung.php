...
class NguoiDung extends Model
{
    ...
    protected $fillable = ['tb1.id','tb1.email','name',
        'tb1.level','tb1.status'];
...
//        if (isset($params['mon_hoc_id'])) {
//            $query->where('tb1.mon_hoc_id', $params['mon_hoc_id']);
//        }
        if (isset($params['search_ten_nguoi_dung']) && strlen($params['search_ten_nguoi_dung']) > 0) {
            $query->where('tb1.name', 'like', '%' . $params['search_ten_nguoi_dung'] . '%');
        }
